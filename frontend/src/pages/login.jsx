  ...
  return (
    <>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media (max-width: 768px) {
          .container { grid-template-columns: 1fr !important; gap: 40px !important; padding: 20px !important; }
          .branding-section { display: none; }
          .login-card { padding: 40px 30px !important; }
        }
      `}</style>
      ...
      <button
        ...
        onMouseEnter={(e) => { e.target.style.background = isDarkMode ? '#3d7a25' : '#e0e8d8'; e.target.style.transform = 'scale(1.05)'; }}
        onMouseLeave={(e) => { e.target.style.background = isDarkMode ? '#2d5016' : '#f0f5e8'; e.target.style.transform = 'scale(1)'; }}
      >
        {isDarkMode ? '☀️' : '🌙'}
      </button>
          ...
          <div style={styles.brandingSection} className="branding-section">
            ...
            <div style={styles.statsContainer}>
              <div style={styles.statItem}>
                <div style={styles.statValue}>{liveStats.accuracy.toFixed(1)}%</div>
                <p style={styles.statLabel}>{t.accuracy}</p>
              </div>
              <div style={styles.statItem}>
                <div style={styles.statValue}>{liveStats.farms.toLocaleString()}</div>
            ...
            <div style={styles.features}>
              {[
                { icon: '🤖', title: 'AI-Powered Insights', desc: 'Get intelligent crop recommendations based on real-time data' },
                { icon: '📊', title: 'Data-Driven Decisions', desc: 'Make informed choices with comprehensive analytics' },
                { icon: '🌱', title: 'Sustainable Farming', desc: 'Maximize yields while supporting environmental health' },
              ].map((feature, idx) => (
                <div key={idx} style={{display: 'flex', gap: '15px', marginTop: '20px', animation: `slideInLeft 0.8s ease-out backwards`, animationDelay: `${0.2 + idx * 0.2}s`}}>
                  <div style={{ width: '45px', height: '45px', background: 'linear-gradient(135deg, #a8d978 0%, #84c34e 100%)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', flexShrink: 0 }}>
                    {feature.icon}
                  </div>
                  <div>
                    <h3 style={{ fontSize: '16px', fontWeight: 600, color: '#2d5016', margin: 0 }}>{feature.title}</h3>
                    <p style={{ fontSize: '13px', color: isDarkMode ? '#aaaaaa' : '#666666', lineHeight: 1.4, margin: 0 }}>{feature.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={styles.loginSection}>
            <div style={styles.loginCard} className="login-card">
              <div style={styles.tabButtons}>
                <button style={styles.tabButton(activeTab === 'login')} onClick={() => setActiveTab('login')}>{t.signIn}</button>
                <button style={styles.tabButton(activeTab === 'signup')} onClick={() => setActiveTab('signup')}>{t.signUp}</button>
              </div>
              ...
              <form onSubmit={handleSubmit}>
                <div style={styles.formGroup}>
                  <label style={styles.label} htmlFor="username">{t.username}</label>
                  <input
                    type="text"
                    id="username"
                ...
                <div style={styles.formGroup}>
                  ...
                  {password && (
                    <>
                      <div style={styles.passwordStrengthBar}><div style={styles.passwordStrengthFill}></div></div>
                      <p style={styles.strengthText}>Password Strength: {getPasswordStrengthText()}</p>
                    </>
                  )}
                </div>
                ...
                <div style={styles.formOptions}>
                  <div style={{display: 'flex', alignItems: 'center', gap: '6px'}}>
                    <input type="checkbox" id="remember" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} style={{ width: '16px', height: '16px', accentColor: '#84c34e', cursor: 'pointer' }} />
                    <label style={{ margin: 0, fontWeight: 400, cursor: 'pointer', fontSize: '13px', color: isDarkMode ? '#aaaaaa' : '#1a1a1a' }} htmlFor="remember">{t.rememberMe}</label>
                  </div>
                  <a href="#" style={{ color: '#4a7c3e', textDecoration: 'none', fontWeight: 500 }} onClick={(e) => e.preventDefault()}>{t.forgotPassword}</a>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '20px' }}>
                  {['Google', 'Email'].map((provider) => (
                    <button key={provider} type="button" style={{ padding: '11px', border: `2px solid ${isDarkMode ? '#404040' : '#e0e8d8'}`, borderRadius: '10px', background: isDarkMode ? '#2d2d2d' : 'white', cursor: 'pointer', color: isDarkMode ? '#aaaaaa' : '#1a1a1a', fontFamily: "'Sora', sans-serif" }}>
                      {provider}
                    </button>
                  ))}
                </div>

                <p style={{ textAlign: 'center', fontSize: '13px', color: isDarkMode ? '#aaaaaa' : '#666666' }}>
                  {t.noAccount} <a href="#" style={{ color: '#2d5016', textDecoration: 'none', fontWeight: 600 }} onClick={(e) => { e.preventDefault(); setActiveTab('signup'); }}>{t.createAccount}</a>
                </p>
              </form>
            </div>
          </div>
